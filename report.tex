...
\begin{itemize}
    ...
    \item \textbf{蒋竺君：}负责消融实验、频域检测器、可视化、答辩材料及报告撰写。具体包括：

    \textbf{消融实验设计与执行：}在队友完成的主复现 baseline 基础上，独立设计三组消融方案——(i)~Noise Mode 消融：对比 $\rho_n=0.2$（开启）与 $\rho_n=0$（关闭）两种训练配置，结合 Neural Cleanse 检测结果分析 Noise Mode 对防御绕过能力的影响；(ii)~形变强度 $s$ 消融：固定 $k=4$，在 $s\in\{0.25,0.50,1.00\}$ 三个取值上分别完成训练与评估；(iii)~控制网格大小 $k$ 消融：固定 $s=0.5$，在 $k\in\{2,4,8\}$ 三个取值上分别完成训练与评估；(iv)~随机种子稳定性验证：在默认参数下使用 seed=0 和 seed=1 完成两组独立训练。共计 10 组消融训练任务，每组均包含模型训练、clean/backdoor/cross 三类测试评估及结果 JSON 记录。实验在租用的 AutoDL 云 GPU 服务器（NVIDIA RTX 3090, Ubuntu 22.04, PyTorch 2.4）上完成，编写了批量运行脚本、tmux 后台任务管理脚本、断点续跑脚本和快速冒烟测试脚本，并针对训练过程中遇到的参数命名冲突、inline Python 转义错误等问题进行了调试修复。

    \textbf{频域检测器设计与实现：}基于 warping 操作在实现层面依赖 grid\_sample 配合双线性插值这一观察，提出从图像频域信号检测 warping 后门痕迹的思路。具体实现包括：(i)~使用已训练的 WaNet 模型对 CIFAR-10 测试集生成 clean/warped 图像对各 500 张；(ii)~对每张图像进行二维 FFT 变换，从幅度谱中提取高频能量占比、频谱熵、频率加权均值、频率加权方差、低频能量占比和中频能量占比共 6 维特征；(iii)~将 1000 个样本按 7:3 划分为训练集和测试集，使用 RBF 核 SVM 训练二分类器；(iv)~在测试集上计算准确率、绘制 ROC 曲线并计算 AUC，与 STRIP 的检测性能（AUC $\approx$ 0.50）进行定量对比。频域检测器在 CIFAR-10 上达到准确率 67.33\%、AUC 0.735。

    \textbf{可视化工作：}完成多类可视化图表，用于报告配图和答辩展示。包括：(i)~warping field 控制网格与变形场热力图；(ii)~原图、warped 后门图、cross 噪声图与残差放大图的对比；(iii)~PSNR 与 SSIM 分布直方图及统计量（均值、方差、最值）；(iv)~不同 $(s,k)$ 参数组合下的变形效果矩阵图；(v)~消融实验三合一汇总图（$s$ 曲线、$k$ 曲线、Noise Mode 柱状图）；(vi)~FFT 频谱对比图（clean vs warped 频谱 + 差异图）；(vii)~六维频域特征分布图；(viii)~频域检测器 ROC 曲线图。所有可视化代码使用 Matplotlib 编写，图片以高分辨率 PNG 输出。

    \textbf{交互式 Dashboard 开发：}使用 HTML/CSS/JavaScript 构建单页 Web 演示平台，用于答辩现场展示。功能包括：(i)~实时 warping 变形交互演示——拖拽滑块调整 $s$ 和 $k$ 参数，Canvas 画布即时渲染原始网格、变形网格和形变热力图；(ii)~频域检测器结果展示——ROC 曲线、FFT 频谱对比和特征分布图的内嵌展示；(iii)~消融实验数据卡片与汇总表展示。Dashboard 采用响应式布局，无需后端服务，浏览器直接打开即可使用，已部署至课程 GitHub 仓库。

    \textbf{答辩材料制作：}制作答辩 PPT（15 页，莫兰迪色系），内容覆盖背景与问题定义、WaNet 方法原理、三阶段工作概览、主实验结果、消融实验分析（Noise Mode / $s$ / $k$）、核心洞察（插值 $\to$ 频域痕迹）、频域检测器设计与结果、全部实验汇总表、结论及成员分工。同时撰写 5 分钟答辩讲稿，按每页 PPT 逐段编排。

    \textbf{报告撰写：}撰写本文第 1 节（引言中消融与频域检测器部分）、第 5--10 节（主实验结果、消融实验分析、频域检测器、总体结果汇总、讨论）及成员分工中本人部分。
\end{itemize}
